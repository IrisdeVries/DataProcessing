...
ApplePie.creator = document.getElementById("header").getElementsByTagName("p");

var number = document.getElementById("ingredient-list").getElementsByTagName("li");
console.log(number);

for (var i = 0; i < number.length; i++)	{
	ApplePie.ingredients.push(number[i].innerHTML);
}
...
for (var i = 0; ApplePie.ingredients.length; i++) {
...
document.getElementsByClassName("pie")[1].addEventListener("click", function()	{
			console.log("click!");
	});
...
function createTransform(domain, range){
	...
	return function(x){
		return alpha * (x - (domain[1] - domain[0]) + beta;
	};
}
